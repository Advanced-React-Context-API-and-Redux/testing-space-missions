...
// grabbed the information to mimic the setMissions setter function in App.js go to --> inspector --> 
...
test('MissionsList shows data when rerendered with new missions data', () => {
    // setting up props from App.js
    const { queryAllByTestId, rerender } = render(<MissionList error='' missions={[]}/>)

    // assert that there are no missions listed when the component first renders
    expect(queryAllByTestId("missions")).toStrictEqual([]);
    expect(queryAllByTestId("missions")).toHaveLength(0)

    
    rerender(<MissionList error='' missions={missions}/>);
